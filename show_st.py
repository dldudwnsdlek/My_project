...
def load_data():
    ...
    dummy_input = np.array([[0 for _ in range(len(field_dims))]], dtype=np.int64)
    model(dummy_input)
...
def get_recom(user, user_non_seen_dict, user_df, movies_df, r_year, r_month, model, label_encoders):
    ...
    merge_data = merge_data[['user_id', 'movie_id','movie_decade', 'year', 'rating_year', 'rating_month', 'rating_decade', 'genre1','genre2', 'genre3', 'gender', 'age', 'occupation', 'zip']]
